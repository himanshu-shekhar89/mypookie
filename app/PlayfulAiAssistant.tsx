...
const activityNames: Record<string, string> = {
  ...
  alwaysyou: "The answer was always you",
  excuse: "Excuse generator",
  roast: "Roast me gently",
  fortune: "Fortune cookie",
  mysterybox: "Mystery box",
};

function lines(items: AiItem[], fallback: string) {
  return (
    items
...
export function PlayfulAiAssistant({
  id,
  relationship,
  config,
  onConfig,
  ...
  function apply(items: AiItem[]) {
    if (id === "quiz") {
    ...
    } else if (id === "wheel" || id === "slots")
      onConfig("prizes", lines(items, config.prizes || ""));
    else if (id === "scratch") {
      onConfig(
        "revealText",
        items[0]?.prompt || config.revealText || "A lovely surprise",
      );
      onConfig(
        "revealDetail",
        items[0]?.options?.[0] || config.revealDetail || "Made just for you",
      );
    } else if (id === "treasure")
      onConfig(
        "treasureClues",
      ...
      onConfig("roasts", lines(items, config.roasts || ""));
    else if (id === "fortune")
      onConfig("fortunes", lines(items, config.fortunes || ""));
    else if (id === "mysterybox")
      onConfig("surprises", lines(items, config.surprises || ""));
    else if (id === "emoji") {
      onConfig("emojiClue", items[0]?.prompt || config.emojiClue || "♡ + ✨");
      onConfig(
        "emojiAnswer",
        items[0]?.options?.[0] || config.emojiAnswer || "our story",
      );
      onConfig(
        "emojiHint",
        items[0]?.options?.[1] || config.emojiHint || "Think of us.",
      );
    }
  }
  ...
  return (
    <section className="playful-ai-assistant expanded">
      <span>✦</span>
      <div className="ai-assistant-copy">
        <strong>
          {fortune ? "Need fresh fortunes?" : "Need fresh ideas?"}
        </strong>
        <small>
          {fortune
            ? "AI writes feel-good fortune statements—never questions. One will be picked at random."
            : "Give this block its own topic. AI will generate editable content around it."}
        </small>
        <label className="ai-topic-field">
          Theme or subject for {activityNames[id] || "this block"}
          <input
            maxLength={180}
            value={config.aiThemePrompt || ""}
      ...
      <button disabled={state === "loading"} onClick={() => void generate()}>
        {state === "loading"
          ? "Dreaming up ideas…"
          : state === "done"
            ? "Added — make them yours ✓"
            : state === "error"
              ? "Try AI again"
              : fortune
                ? "Generate themed fortunes"
                : "Generate around this theme"}
      </button>
    </section>
  );
}
