...
type CheckoutBlock={instanceId?:string;id:string;icon:string;name:string;price:number;color:string};
...
export function CheckoutPage({blocks,senderName,name,occasion,subtotal,revealAt,onRevealAt,onBack,onQuote,onCreateOrder,onVerifyPayment,onDemoComplete}:{blocks:CheckoutBlock[];senderName:string;name:string;occasion:string;subtotal:number;revealAt:string;onRevealAt:(value:string)=>void;onBack:()=>void;onQuote:(coupon:string)=>Promise<CouponQuote|null>;onCreateOrder:(coupon:string)=>Promise<PaymentOrder|null>;onVerifyPayment:(orderId:string,result:RazorpayResult)=>Promise<string|null>;onDemoComplete:(orderId:string)=>Promise<string|null>}){
