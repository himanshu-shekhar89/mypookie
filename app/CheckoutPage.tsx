...
export function CheckoutPage({blocks,senderName,name,occasion,subtotal,revealAt,onRevealAt,compatibilityPin,onCompatibilityPin,onBack,onQuote,onCreateOrder,onVerifyPayment,onDemoComplete}:{blocks:CheckoutBlock[];senderName:string;name:string;occasion:string;subtotal:number;revealAt:string;onRevealAt:(value:string)=>void;compatibilityPin:string;onCompatibilityPin:(value:string)=>void;onBack:()=>void;onQuote:(coupon:string)=>Promise<CouponQuote|null>;onCreateOrder:(coupon:string)=>Promise<PaymentOrder|null>;onVerifyPayment:(orderId:string,result:RazorpayResult)=>Promise<string|null>;onDemoComplete:(orderId:string)=>Promise<string|null>}){
