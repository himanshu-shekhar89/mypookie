...
export function TinyBlockCustomization({id,config,onConfig}:Props){
  if(id==="wouldrather"){
  ...
  if(id==="truthdare")return <Section title="Truth or Dare Roulette" hint="The wheel chooses a type, then one of your prompts"><Lines label="Truth prompts" configKey="truths" value={config.truths||""} max={8} onConfig={onConfig}/><Lines label="Dare prompts" configKey="dares" value={config.dares||""} max={8} onConfig={onConfig}/></Section>;
  ...
  if(id==="groupboard")return <GroupBoardEditor config={config} onConfig={onConfig}/>;
  return null;
}
...
function GroupBoardEditor({config,onConfig}:{config:Record<string,string>;onConfig:Props["onConfig"]}){
