...
function BondQuestionEditor({config,onConfig}:{config:Record<string,string>;onConfig:Props["onConfig"]}){
  ...
  const patchQuestion=(index:number,value:string)=>onConfig("bondQuestions",JSON.stringify(normalizedQuestions.map((item,itemIndex)=>itemIndex===index?value:item)));
  ...
  return <div className="bond-question-editor"><div className="bond-how"><span>1</span><p>You answer these now.</p><span>2</span><p>They answer the same set while opening the gift.</p><span>3</span><p>AI compares both perspectives and creates the reveal.</p></div>{normalizedQuestions.map((question,index)=><article key={index}><label className="field">Question {index+1}<input maxLength={110} value={question} onChange={event=>patchQuestion(index,event.target.value)}/></label><label className="field">Your answer<textarea rows={2} maxLength={180} value={normalizedAnswers[index]} onChange={event=>patchAnswer(index,event.target.value)} placeholder="Answer honestly—the contrast makes the result special."/><small>{normalizedAnswers[index].length}/180</small></label></article>)}</div>;
}
