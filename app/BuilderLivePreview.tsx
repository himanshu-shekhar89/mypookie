...
export function BuilderLivePreview({ block, name, theme, ambience, onInteract, onComplete, onReward }: { block: PreviewBlock; name: string; theme: string; ambience: string; onInteract?: () => void; onComplete?: () => void; onReward?: (reward:string) => void }) {
  ...
  return (
    <div className={`builder-live-card live-theme-${theme.toLowerCase().replaceAll(" ", "-")}`}>
      ...
      <div className="live-interaction" onClickCapture={onInteract} onPointerDownCapture={onInteract}>
        ...
        {["wouldrather","neverhave","truthdare","tapheart","matchpair","countdownus","constellation","growthring","movie","alwaysyou","excuse","roast","fortune","mysterybox","playlist","countdowninvite","groupboard"].includes(block.id) && <TinyBlockPreview id={block.id} config={config} onComplete={onComplete} onReward={onReward} />}
