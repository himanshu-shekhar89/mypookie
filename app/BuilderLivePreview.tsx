...
import { signalGiftMedia } from "./GiftSoundtrack";
...
export function BuilderLivePreview({
  ...
  const customWheelPrizes = (
    config.prizes || wheelPrizes.slice(0, 5).join("\n")
  )
    .split("\n")
    .map((item) => item.trim())
    .filter(Boolean)
    .slice(0, 5);
  const wheelOptions =
    customWheelPrizes.length >= 2 ? customWheelPrizes : wheelPrizes.slice(0, 5);
  ...
  function spinWheel() {
    ...
    const planned = (config.plannedResults || "")
      .split("\n")
      .map((item) => item.trim());
    const plannedIndex =
      config.resultMode === "Plan every spin"
        ? wheelOptions.findIndex(
            (item) =>
              item.toLowerCase() ===
              String(planned[wheelSpinCount] || "").toLowerCase(),
          )
        : -1;
    const randomValue = window.crypto.getRandomValues(new Uint32Array(1))[0];
    const winner =
      plannedIndex >= 0 ? plannedIndex : randomValue % wheelOptions.length;
    setSpinning(true);
  ...
  const envelopeStyle = (config.envelopeStyle || "Blush satin")
    .toLowerCase()
    .replaceAll(" ", "-");
  const sealStyle = (config.envelopeSeal || "Wax heart")
    .toLowerCase()
    .replaceAll(" ", "-")
    .replaceAll("/", "-");
  const pageStyle = (config.pageType || "Classic cream")
    .toLowerCase()
    .replaceAll(" ", "-");
  const fontStyle = (config.letterFont || "Handwritten")
    .toLowerCase()
    .replaceAll(" ", "-");
  const stampSymbols: Record<string, string> = {
    "Rose stamp": "❀",
    "Air mail": "✈",
    "Golden heart": "♥",
    Postmark: "◎",
  ...
  const stickerSymbols: Record<string, string> = {
    Daisies: "✿ ❀",
    "Heart cluster": "♥ ♡",
    Stars: "✦ ✧",
    Smiley: "☺",
  ...
  const sealSymbols: Record<string, string> = {
    "Wax heart": "♥",
    "Monogram wax": (senderName || "M").slice(0, 1).toUpperCase(),
    "Flower sticker": "✿",
    "Star sticker": "✦",
    "Glue strip": "—",
    None: "",
  ...
  return (
    <div
      className={`builder-live-card live-theme-${theme.toLowerCase().replaceAll(" ", "-")}`}
    >
      {ambience === "Petals" && (
        <div className="live-ambience petals" aria-hidden="true">
          <i>✿</i>
          <i>·</i>
          <i>✿</i>
          <i>·</i>
      ...
      {ambience === "Soft sparkles" && (
        <div className="live-ambience sparkles" aria-hidden="true">
          <i>✦</i>
          <i>✧</i>
          <i>✦</i>
          <i>✧</i>
      ...
      <div className="live-recipient-label">
        A LITTLE SOMETHING FOR {name.toUpperCase()}
      </div>
      ...
      >
        {block.id === "letter" && (
          ...
          >
            <div className="letter-particles" aria-hidden="true">
              {Array.from({ length: letterDensity }, (_, index) => (
                <i
                  key={index}
                  style={
                    {
                      "--angle": `${index * (360 / letterDensity)}deg`,
                      "--distance": `${70 + (index % 5) * 18}px`,
            ...
            <button
              className={`envelope-3d ${opened ? "opened" : ""} ${letterStage === "revealed" ? "message-ready" : ""}`}
              onClick={letterStage === "closed" ? openLetter : resetLetter}
              aria-label={
                letterStage === "closed"
                  ? "Open the envelope"
                  : "Close and replay the letter"
              }
            >
              <span className="envelope-card">
                ...
                <span className="envelope-face envelope-front">
                  ...
                  <strong>{config.frontText || `For ${name}`}</strong>
                  <em className="envelope-stickers">
                    {stickerSymbols[config.stickerStyle || "Daisies"]}
                  </em>
                  <b className="envelope-seal">
                    {sealSymbols[config.envelopeSeal || "Wax heart"]}
                  </b>
                </span>
                <span className="envelope-face envelope-back">
                  <i className="envelope-flap" />
                  <strong>
                    {config.backText ||
            ...
            <small className="letter-effect-caption">
              {letterStage === "closed"
                ? "Tap the rotating envelope"
                : letterStage === "burst"
                  ? "A little magic is unfolding…"
                  : "Tap to close and replay"}
            </small>
          </div>
        )}
        {block.id === "voice" && (
          <VoicePreview
            ...
            playerStyle={config.playbackStyle}
            onComplete={onComplete}
          />
        )}
        {block.id === "video" && (
          <VideoNotePlay config={config} onComplete={onComplete} />
        )}
        {block.id === "flowers" && (
          <EGiftPreview config={config} onComplete={onComplete} />
        )}
        {block.id === "quiz" && (
          <QuizPlay
            config={config}
        ...
        {block.id === "wheel" && (
          <div className={`live-wheel-scene casino-wheel-scene frame-${(config.wheelFrame || "Lucky carnival").toLowerCase().replaceAll(" ", "-")}`}>
              ...
              >
                {wheelOptions.map((prize, index) => (
                  <span
                    key={`${prize}-${index}`}
                    style={{
                      transform: `rotate(${index * wheelSlice + wheelSlice / 2}deg) translateY(-82px)`,
                    }}
                  >
                    {prize}
                  </span>
            ...
            <button
              onClick={spinWheel}
              disabled={
                spinning || wheelSpinCount >= (Number(config.spins) || 1)
              }
            >
              {spinning
                ? "Spinning…"
                : wheelSpinCount >= (Number(config.spins) || 1)
                  ? "No spins left"
                  : "Spin the wheel"}
            </button>
            <output>
              {wheelResult} ·{" "}
              {Math.max((Number(config.spins) || 1) - wheelSpinCount, 0)} left
            </output>
          </div>
        )}
        {block.id === "slots" && (
        ...
        {block.id === "puzzle" && (
          <PhotoPuzzlePlay
            key={`${config.difficulty}-${config.imageUrl}`}
            config={config}
            onComplete={onComplete}
            onReward={onReward}
          />
        ...
        {block.id === "memory" && (
          <MemoryBook config={config} onComplete={onComplete} />
        )}
        {block.id === "scratch" && (
          <ScratchPreview
            revealText={config.revealText}
            revealDetail={config.revealDetail}
      ...
      <div className="live-device-hint">
        This is what {name || "your recipient"} will interact with
      </div>
    </div>
  );
}
...
function EGiftPreview({
  ...
  return (
    <button
      className={`egift-stage celebration-scene scene-${scene.toLowerCase().replaceAll(" ", "-")} ${playing ? "playing" : ""} intensity-${(config.intensity || "Lush").toLowerCase()}`}
      onClick={() => {
        if (!playing) {
            ...
            style={
              {
                ...
                "--drift": `${(index % 2 ? 1 : -1) * (18 + (index % 5) * 7)}px`,
              } as React.CSSProperties
            }
          >
...
function VideoNotePlay({
  ...
  const frameClass = frame
    ...
    .replaceAll(" ", "-")
    .replace("&", "and");
  const captionFont = (config.videoCaptionFont || "Handwritten")
    .toLowerCase()
    .replaceAll(" ", "-");
  const shower = config.videoShower || "Petal shower";
  const showerClass = shower.toLowerCase().replaceAll(" ", "-");
  const showerSymbols =
    shower === "Flower shower"
      ? ["✿", "❀", "❁"]
      : shower === "Heart shower"
        ? ["♥", "♡", "♥"]
        : shower === "Golden sparkles"
          ? ["✦", "✧", "⋆"]
          : ["❀", "·", "❁"];
  const density =
    shower === "None"
      ? 0
      : Math.min(36, Math.max(8, Number(config.videoShowerDensity) || 18));
  useEffect(() => () => signalGiftMedia(false), []);
  return (
    <div
      className={`video-note-player video-frame-${frameClass} caption-font-${captionFont}`}
      style={
        {
          "--video-caption": config.videoCaptionColor || "#3f3036",
        } as React.CSSProperties
      }
    >
      {density > 0 && (
        <div
          className={`video-shower shower-${showerClass} ${playing ? "active" : ""}`}
          aria-hidden="true"
        >
              ...
              style={
                {
                  ...
                  "--sway": `${(index % 2 ? 1 : -1) * (18 + (index % 6) * 6)}px`,
                } as React.CSSProperties
              }
            >
              {showerSymbols[index % showerSymbols.length]}
            </i>
          ))}
        </div>
      )}
      {config.videoUrl ? (
        <>
          <div className="video-screen">
            ...
            {frame === "Retro cam" && (
              <>
                <span className="retro-rec">● REC</span>
                <span className="retro-date">MYPOOKIE · MEMORY</span>
                <i className="retro-scan" />
              </>
            )}
          </div>
          {failed && (
            <p className="video-playback-error">
              This video format cannot play in this browser. Upload an MP4, MOV
              or WebM file from the editor.
            </p>
          )}
          <strong>
            {config.videoCaption ||
              "A little face-to-face moment, just for you."}
          </strong>
          <small>Watch to the end to continue</small>
        </>
      ) : (
        <div className="video-note-empty">
          <span>▶</span>
          <strong>Your video note will appear here</strong>
          <small>Choose a finished video from your gallery.</small>
        </div>
      )}
    </div>
  );
...
function ThisOrThatPlay({
  ...
  const rounds = parseJson<ThisOrThatRound[]>(config.thisOrThatRounds, [
  ...
  if (index >= rounds.length)
    return (
      <div className="this-or-that-finish">
        <span>♡</span>
        <strong>Your little favourites</strong>
        {choices.map((choice, choiceIndex) => (
          <small key={choiceIndex}>
            {rounds[choiceIndex]?.prompt}: <b>{choice}</b>
          </small>
        ))}
      </div>
    );
  ...
  async function saveChoices(next: string[]) {
    ...
    await fetch(
      `${process.env.NEXT_PUBLIC_API_URL || "https://backend-production-22bd.up.railway.app"}/api/public/gifts/${giftId}/responses`,
      {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          responseType: "THIS_OR_THAT",
          contributorName: recipientName || "Recipient",
          responseText: JSON.stringify({ choices: next }),
          photoUrls: [],
        }),
      },
    ).catch(() => {});
  ...
  function choose(value: string) {
    ...
    if (index === rounds.length - 1) {
      ...
      playSound("win");
      onReward?.(`This or that: ${value}`);
      onComplete?.();
    }
    setIndex((current) => current + 1);
  }
  return (
    <div className="this-or-that-play">
      <div className="quiz-dots">
        {rounds.map((_, dot) => (
          <i
            key={dot}
            className={dot < index ? "done" : dot === index ? "current" : ""}
          />
        ))}
      </div>
      <small>QUICK CHOICE {index + 1}</small>
      <strong>{round.prompt}</strong>
      <div>
        <button onClick={() => choose(round.left)}>{round.left}</button>
        <span>OR</span>
        <button onClick={() => choose(round.right)}>{round.right}</button>
      </div>
    </div>
  );
}
...
function EmojiDecoderPlay({
  ...
  return (
    <div className="emoji-decoder-play">
      <small>DECODE THE MEMORY</small>
      <strong>{config.emojiClue || "☕ + 🌧 + ♡"}</strong>
      {hint && (
...
function HeartCatchPlay({
  ...
  const target = Math.min(Math.max(Number(config.target) || 6, 3), 10);
  const [caught, setCaught] = useState(0);
  const left = 8 + ((caught * 37) % 78);
  const top = 12 + ((caught * 53) % 64);
  function catchHeart() {
    const next = caught + 1;
    setCaught(next);
    playSound("tile");
    if (next >= target) {
      playSound("win");
      onReward?.(config.prize || "A pocketful of kisses");
      onComplete?.();
    }
  }
  return (
    <div className={`heart-catch-play ${caught >= target ? "finished" : ""}`}>
      <header>
        <span>
          {caught}/{target}
        </span>
        <strong>
          {caught >= target ? "Prize unlocked!" : "Catch every heart"}
        </strong>
      </header>
      {caught < target ? (
        <div className="heart-catch-stage">
          <button
        ...
        <div className="heart-catch-prize">
          <span>♡</span>
          <strong>{config.prize || "A pocketful of kisses"}</strong>
        </div>
      )}
    </div>
  );
...
function QuizPlay({
  ...
  if (index >= questions.length)
    return (
      <div className="quiz-finished">
        ...
        <strong>You know this story beautifully ♡</strong>
        <button
          onClick={() => {
            hiddenOptionsRef.current = new Set();
            setHiddenOptions(new Set());
  ...
  if (!question)
    return <div className="quiz-finished">Add a question on the right.</div>;
  function hideFromCursor(event: React.PointerEvent<HTMLDivElement>) {
    ...
    const buttons = Array.from(
      stageElement.querySelectorAll<HTMLButtonElement>("button[data-option]"),
    ).map((button) => ({
    ...
    buttons.forEach(({ optionIndex, rect }) => {
      ...
      if (
        Math.hypot(pointer.x - nearestX, pointer.y - nearestY) < 64 &&
        !next.has(optionIndex)
      ) {
  ...
  function answer(optionIndex: number) {
    ...
    window.setTimeout(() => {
      if (index < questions.length - 1) {
        ...
        setFeedback("");
        setOrder(
          questions[nextIndex].options.map(
            (_, optionPosition) => optionPosition,
          ),
        );
      } else {
  ...
  const visibleOrder = [
    ...
    ...question.options
      ...
      .filter((optionIndex) => !order.includes(optionIndex)),
  ];
  return (
    <div className="advanced-quiz">
      ...
      <div className="runaway-stage" onPointerMove={hideFromCursor}>
        {visibleOrder.map((optionIndex) => {
...
function SlotMachinePlay({
  ...
  function pull() {
    ...
    setResult("The reels are spinning…");
    const timer = window.setInterval(
      () =>
        setReels((current) =>
          current.map((reel, index) =>
            index < stoppedReelsRef.current
              ? reel
    ...
    const stopReel = (index: number, label: string) =>
      window.setTimeout(
        () => {
    ...
    timersRef.current = [
      stopReel(0, "First reel locked…"),
      stopReel(1, "Two reels match…"),
    ];
    timersRef.current.push(
      window.setTimeout(() => {
        window.clearInterval(timer);
        intervalRef.current = null;
        stoppedReelsRef.current = 3;
        setStoppedReels(3);
  ...
  return (
    <div className="live-slot-machine casino-slot-scene">
      <div className="slot-machine-rig">
        <div className={`slot-machine-reels ${rolling ? "rolling" : ""}`}>
          {reels.map((reel, index) => (
            <i
              key={index}
              className={
                rolling && index >= stoppedReels ? "spinning" : "stopped"
              }
            >
        ...
        <button
          className={`slot-machine-lever ${rolling ? "pulled" : ""}`}
          onClick={pull}
          disabled={rolling || pulls >= maxPulls}
...
function shuffleTiles(size: number) {
  ...
  if (values.every((tile, index) => tile === index))
    [values[0], values[1]] = [values[1], values[0]];
  return values;
}

function PhotoPuzzlePlay({
  config,
  onComplete,
  onReward,
}: {
  ...
  function moveTile(index: number) {
    ...
    const distance =
      Math.abs(Math.floor(picked / size) - Math.floor(index / size)) +
      Math.abs((picked % size) - (index % size));
    if (distance === 1) {
      const next = [...tiles];
      ...
      if (next.every((tile, tileIndex) => tile === tileIndex)) {
        ...
        playSound("win");
        onReward?.(
          `Completed the photo puzzle in ${nextMoves} move${nextMoves === 1 ? "" : "s"}`,
        );
        onComplete?.();
  ...
  function finishPuzzle(finalMoves: number, auto = false) {
    ...
    onReward?.(
      auto
        ? "Auto-solved the photo puzzle"
        : `Completed the photo puzzle in ${finalMoves} move${finalMoves === 1 ? "" : "s"}`,
    );
    onComplete?.();
  ...
  function autoSolve() {
    ...
    for (let target = 0; target < working.length; target++) {
      ...
      while (Math.floor(position / size) > Math.floor(target / size)) {
        const next = position - size;
        swaps.push([position, next]);
        [working[position], working[next]] = [working[next], working[position]];
        position = next;
      }
      while (Math.floor(position / size) < Math.floor(target / size)) {
        const next = position + size;
    ...
    playSound("reveal");
    if (!swaps.length) {
      finishPuzzle(moves, true);
      return;
    }
    swaps.forEach(([from, to], index) =>
      autoTimers.current.push(
        window.setTimeout(() => {
          ...
          if (index === swaps.length - 1)
            finishPuzzle(moves + swaps.length, true);
        }, index * 95),
      ),
    );
  }
  ...
  if (phase === "photo")
    return (
      <div className="puzzle-complete-photo">
        <img src={image} alt="Completed puzzle" />
        <strong>Perfect! ✦</strong>
        <span>
          {config.successMessage || "You put this memory back together."}
        </span>
        <b className="puzzle-move-result">
    ...
    >
      ...
      <div>
        ...
        >
          ...
          {phase === "perfect" && (
            <div className="live-puzzle-success">
              Perfect! ✦
              <small>
                {autoSolving
                  ? "Putting your memory together…"
                  : `Solved in ${moves} move${moves === 1 ? "" : "s"}`}
              </small>
            </div>
          )}
  ...
  return (
    ...
    >
      ...
      <div className="book-controls">
        ...
        <span>
          {page + 1}/{pages.length} · turn the album page
        </span>
        <button onClick={() => turn(1)}>→</button>
      </div>
    </div>
...
function TreasurePlay({
  ...
  const clues = parseJson<TreasureClue[]>(config.treasureClues, [
    {
      clue: "Start where we first said hello.",
      hint: "Think of our favourite café.",
      answer: "cafe",
      photo: "",
      caption: "",
    },
  ...
  if (index >= clues.length)
    return (
      <div className="treasure-winner">
        ...
        <p>{config.finalSurprise || "A mystery date for us"}</p>
        <button
          onClick={() => {
            setIndex(0);
            setAnswer("");
  ...
  return (
    <div className="advanced-treasure">
      ...
      {stage === "clue" ? (
        <div className="treasure-clue-stage">
          {current.photo && (
            <figure>
              <img src={current.photo} alt="A visual clue" />
              {current.caption && <figcaption>{current.caption}</figcaption>}
            </figure>
          )}
          <strong>{current.clue}</strong>
          <button onClick={() => setStage("answer")}>
            Next · answer this clue →
          </button>
        </div>
      ) : (
        <div className="treasure-answer-stage">
          <strong>{current.clue}</strong>
          {showHint && (
            <p>
              Hint: {current.hint || "No hint for this one — you’ve got this!"}
            </p>
          )}
          <input
            autoFocus
          ...
          />
          <div>
            <button onClick={() => setStage("clue")}>← Back to clue</button>
...
function CalendarPlay({
  ...
  useEffect(() => {
    const refresh = () => setToday(new Date().toISOString().slice(0, 10));
    refresh();
  }, []);
  ...
  const elapsed =
    start && current
      ...
      : 1;
  const available =
    config.unlockRule === "Recipient can open anytime"
      ? days
      : Math.min(Math.max(elapsed, 1), days);
  function openDay(index: number) {
    if (index >= available) return;
    const isNew = !opened.includes(index);
    setOpened((current) =>
    ...
    playSound("reveal");
    if (isNew) {
      onReward?.(`Day ${index + 1}: ${notes[index]}`);
      onComplete?.();
    }
  }
  return (
    <div className="calendar-experience">
      <header>
        ...
        <div>
          <small>
            {config.unlockRule === "Recipient can open anytime"
              ? "OPEN ANYTIME"
              : "ONE NEW DOOR EACH DAY"}
          </small>
          <strong>
            {available} of {days} {available === 1 ? "day is" : "days are"}{" "}
            ready
          </strong>
        </div>
      </header>
      <p>
        {config.unlockRule === "Recipient can open anytime"
          ? "Every surprise is available now. Tap any numbered door."
          : "Come back each day to unlock another note, memory or surprise."}
      </p>
      <div className="calendar-doors">
        {notes.map((_, index) => (
          <button
            key={index}
            className={`${opened.includes(index) ? "opened" : ""} ${index >= available ? "locked" : ""}`}
            onClick={() => openDay(index)}
            aria-label={
              index >= available
                ? `Day ${index + 1} is locked`
                : `Open day ${index + 1}`
            }
          >
            <span>{index >= available ? "⌾" : index + 1}</span>
            <b>{opened.includes(index) ? "♥" : "✦"}</b>
          </button>
        ))}
      </div>
      <div className={`calendar-reveal ${selectedDay === null ? "empty" : ""}`}>
        {selectedDay === null ? (
          <>
            <small>HOW IT WORKS</small>
            <strong>Tap an available door to reveal today’s message.</strong>
          </>
        ) : (
          <>
            <small>DAY {selectedDay + 1}</small>
            <strong>{notes[selectedDay]}</strong>
          </>
        )}
      </div>
    </div>
...
function GiftCardPlay({
  ...
  return (
    <button
      className={`advanced-gift-card ${revealed ? "revealed" : ""} ${isFlip ? "flip-mode" : "blur-mode"}`}
      onClick={() => {
        if (!revealed) {
          playSound("reveal");
          onReward?.(config.value || config.brand || "Gift card revealed");
          onComplete?.();
        }
        setRevealed((value) => !value);
      }}
    >
      <div className="gift-face gift-front">
        <small>MYPOOKIE. PRESENTS</small>
        <strong>{config.brand || "A gift for you"}</strong>
        <span>{isFlip ? "Tap to flip" : "Tap to bring it into focus"}</span>
      </div>
      <div className="gift-face gift-back">
        <small>
          {config.showValue !== "false" ? config.value : "A LITTLE SOMETHING"}
        </small>
        {config.showCode !== "false" && (
          <strong>{config.code || "POOKIE-LOVE-24"}</strong>
        )}
...
function GiftScratchCard({
  ...
  return (
    <div className="gift-scratch-card">
      <div>
        <small>
          {config.showValue !== "false" ? config.value : "MYPOOKIE. GIFT"}
        </small>
        {config.showCode !== "false" && <strong>{config.code}</strong>}
        {config.showNote !== "false" && <span>{config.giftMessage}</span>}
      </div>
      <ScratchSurface
...
function VoicePreview({
  ...
  const style = (playerStyle || "Classic waveform")
    .toLowerCase()
    .replaceAll(" ", "-");
  useEffect(() => () => signalGiftMedia(false), []);
  function toggle() {
  ...
  return (
    <div
      className={`live-voice voice-style-${style} ${playing ? "playing" : ""}`}
    >
      <button
        onClick={toggle}
        ...
        <span>
          {audioUrl
            ? playing
              ? "Playing now"
              : duration
                ? `${Math.ceil(duration)} seconds · ready`
                : "Ready to play"
            : "Record on the right"}
        </span>
        <input
...
function ScratchSurface({
    ...
    function paint() {
      ...
      paintColors.forEach((color, index) =>
        gradient.addColorStop(
          index / Math.max(paintColors.length - 1, 1),
          color,
        ),
      );
  ...
  function scratch(event: React.PointerEvent<HTMLCanvasElement>) {
    ...
    const columns = 12,
      rows = 6,
      cellX = Math.floor(point.x / Math.max(rect.width / columns, 1)),
      cellY = Math.floor(point.y / Math.max(rect.height / rows, 1));
    for (let x = cellX - 1; x <= cellX + 1; x++)
      for (let y = cellY - 1; y <= cellY + 1; y++)
        if (x >= 0 && x < columns && y >= 0 && y < rows)
          touchedCells.current.add(`${x}:${y}`);
    context.save();
    ...
    playSound("scratch");
    if (touchedCells.current.size / (columns * rows) >= 0.3) reveal();
  }
  ...
  function finish(event: React.PointerEvent<HTMLCanvasElement>) {
    ...
    if (!revealed && clear / Math.max(total, 1) > 0.22) reveal();
  }
  ...
  return (
    <canvas
      width={900}
      height={350}
      className={revealed ? "scratch-complete" : ""}
      ref={canvasRef}
      onPointerDown={(event) => {
...
function ScratchPreview({
  ...
  return (
    <div className="live-scratch">
      <div>
        <small>YOU UNLOCKED</small>
        <strong>{revealText || "A candlelit dinner ♡"}</strong>
        <span>{revealDetail || "Friday · 8:00 PM"}</span>
      </div>
      <ScratchSurface
        label="SCRATCH HERE"
        colors={
          palettes[coating || "Lilac shimmer"] || palettes["Lilac shimmer"]
        }
        onReveal={() => {
          playSound("win");
          onReward?.(revealText || "Scratch surprise revealed");
          onComplete?.();
        }}
      />
    </div>
