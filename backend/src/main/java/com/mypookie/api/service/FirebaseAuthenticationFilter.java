...
public class FirebaseAuthenticationFilter extends OncePerRequestFilter {
 ...
 @Override protected void doFilterInternal(HttpServletRequest req,HttpServletResponse res,FilterChain chain)throws ServletException,IOException{
  ...
  try{
   if(enabled&&auth!=null&&auth.startsWith("Bearer ")){FirebaseToken token=FirebaseAuth.getInstance(firebaseApp.getObject()).verifyIdToken(auth.substring(7));authenticate(token.getUid(),token.getEmail(),isAdminEmail(token.getEmail()));}
