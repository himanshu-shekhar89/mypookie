...
public class GiftService {
 ...
 private int calculate(String blocksJson){
  try{Set<String> ids=new HashSet<>();json.readTree(blocksJson).forEach(n->ids.add(n.path("id").asText()));return activities.findAllById(ids).stream().mapToInt(ActivityType::getPricePaise).sum();}
  catch(Exception e){throw new IllegalArgumentException("Invalid blocks JSON");}
 }
 public Gift publish(String senderId,String id){Gift g=gifts.findById(id).orElseThrow();if(!g.getSenderId().equals(senderId))throw new SecurityException();g.setStatus("PUBLISHED");g.setShareToken(UUID.randomUUID().toString().replace("-",""));return gifts.save(g);}
}
