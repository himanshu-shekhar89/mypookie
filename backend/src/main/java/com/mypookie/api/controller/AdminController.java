...
public class AdminController {
 ...
 public AppUser role(@PathVariable String id,@RequestBody RoleUpdate request){
  if(!Set.of("USER","ADMIN").contains(request.role()))throw new IllegalArgumentException("Invalid role");
  var user=users.findById(id).orElseThrow();user.setRole(request.role());return users.save(user);
 }
